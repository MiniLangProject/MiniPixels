...
The native CLI currently provides `info`, `doctor`, `validate`, and `new`. The legacy Python tool still owns `build`, `run`, `generate`, and `package` while PNG and compiler process launching are moved into MiniLang.
