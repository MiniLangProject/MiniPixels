...
An unprotected `.mpx` file starts with `MPX1`, followed by a little-endian entry table and payload blocks. The former reserved entry byte identifies raw, Deflate, or dependency-free RLE storage; repeated entries may reference the same exact block. The MPX1 loader discovers the table boundary from the first canonical payload and parses the complete index from one bounded read for normal-sized indexes, rather than issuing per-entry reads. Both MPX1 and MPX3 payloads stay file-backed until first access. The protected loader accepts only MPX3 version 4; MPX2 and older MPX3 versions are rejected. Authentication precedes bounded decompression. Names resolve through a hash index, generated accessors bypass it with stable slots, and per-slot arrays cache logical payloads/images in constant time. Successfully decoded PNG, text, and data entries release their source-byte cache. Counters expose cache hits, misses, retained raw bytes, physical bytes read, logical bytes decoded, and bulk-read counts.

Explicit and resident preloading sort uncached slots into file order and coalesce adjacent ranges into bounded sequential reads (16 MiB by default). The temporary batch buffer is released after its entries reach their final per-slot representation, avoiding a permanent second in-memory copy of the complete MPX file. Entries that reference a deduplicated physical block also share its decoded buffer instead of reading and decompressing it again. Unprotected Deflate ranges are passed directly from that batch to the native runtime and inflated into exact-sized final buffers; per-entry AES-GCM authentication remains ahead of decompression for protected packs. Per-project and per-entry compression profiles allow a compact default while latency-critical prepared data can remain raw. The PNG runtime handles stored/fixed/dynamic Deflate blocks, filters 0 through 4, and standard 8-bit color types plus 1/2/4-bit palettes; Adam7 interlace remains unsupported.
